...
public final class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length <= 0) {
            ...
            System.out.println("org.traccar.Main.main()"+configFile);
    ...
    private static void scheduleDatabaseCleanup() {
        new Timer().scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                try {
   final DSCHisory dsc1 = new DSCHisory();
        
        dsc1.setJar_version("1.0");
        dsc1.setJar_info("add command history");
        dsc1.setJar_useinfo("Truetrack_Listener");
       
        dsc1.setJar_name("schol_pro_20.0.jar");
        Calendar now = Calendar.getInstance();
      TimeZone timeZone = now.getTimeZone();

      String zone=timeZone.getDisplayName();
     
                    Context.getDataManager().clearHistory();
 //  if(!zone.equals("India Standard Time") || zone != "India Standard Time") {
                                          Context.getDataManager().AddDscHistoryVersion(dsc1);
                                  //   }
