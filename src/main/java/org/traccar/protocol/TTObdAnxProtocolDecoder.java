...
//import org.traccar.helper.PrintOut;

public class TTObdAnxProtocolDecoder extends BaseProtocolDecoder {

    public TTObdAnxProtocolDecoder(Protocol protocol) {
        super(protocol);
    }
    ...
    public static final int MSG_OIL_CONTROL = 0XA006;
     public static final int MSG_TERMINAL_ATTRIBUTE = 0X0107;
...
//            System.out.println("alaram ALARM_Crash-----");
       ...
       private void gnssModule(Position position,long value) {
//         System.out.println("gnssModule value-----"+value);
        if (BitUtil.check(value, 0)) {
//              System.out.println("alaram sos-----");
           position.set("gnssModule", "GPS positioning is supported");
        }else{
             position.set("gnssModule", "GPS positioning is not supported");
        }
        if (BitUtil.check(value, 1)) {
//              System.out.println("alaram ALARM_OVERSPEED-----");
             position.set("gnssModule", "Support for Beidou positioning");
        }else{
             position.set("gnssModule", "No support for Beidou positioning");
        }
        
        if (BitUtil.check(value, 2)) {
//              System.out.println("alaram Tired-----");
             position.set("gnssModule", "GLONASS positioning is supported");
        }else{
              position.set("gnssModule", "GLONASS positioning is not supported");
        }
     
        if (BitUtil.check(value, 3)) {
//               System.out.println("alaram ALARM_GPS_ANTENNA_CUT-----");
             position.set("gnssModule","Galileo positioning is  supported");
        }else{
             position.set("gnssModule","Galileo positioning is not supported");
        }
      
    }
       
       private void communicationModule(Position position,long value) {
//        System.out.println("communicationModule value-----"+value);
        if (BitUtil.check(value, 0)) {
//              System.out.println("alaram sos-----");
           position.set("communicationModule", "GPRS communication is supported");
        }else{
             position.set("communicationModule", "GPRS communication is not supported");
        }
        if (BitUtil.check(value, 1)) {
//              System.out.println("alaram ALARM_OVERSPEED-----");
             position.set("communicationModule", "CDMA communications are supported");
        }else{
             position.set("communicationModule", "CDMA communications are not supported");
        }
        
        if (BitUtil.check(value, 2)) {
//              System.out.println("alaram Tired-----");
             position.set("communicationModule", "TD-SCDMA communication is supported");
        }else{
              position.set("communicationModule", "TD-SCDMA communication is not supported");
        }
     
        if (BitUtil.check(value, 3)) {
//               System.out.println("alaram ALARM_GPS_ANTENNA_CUT-----");
             position.set("communicationModule","WCDMA communication is supported");
        }else{
             position.set("communicationModule","WCDMA communication is not supported");
        }
        
         if (BitUtil.check(value, 4)) {
//               System.out.println("alaram ALARM_GPS_ANTENNA_CUT-----");
             position.set("communicationModule","CDMA2000 communication is supported");
        }else{
             position.set("communicationModule","CDMA2000 communication is not supported");
        }
         
          if (BitUtil.check(value, 5)) {
//               System.out.println("alaram ALARM_GPS_ANTENNA_CUT-----");
             position.set("communicationModule","TD-LTE communication is supported");
        }else{
             position.set("communicationModule","TD-LTE communication is not supported");
        }
          
           if (BitUtil.check(value, 6)) {
//               System.out.println("alaram ALARM_GPS_ANTENNA_CUT-----");
             position.set("communicationModule","reserved");
        }
           
         if (BitUtil.check(value, 7)) {
//               System.out.println("alaram ALARM_GPS_ANTENNA_CUT-----");
             position.set("communicationModule","Other communication methods are supported");
        }else{
             position.set("communicationModule","Other communication methods are not supported");
        }
...
//         System.out.println("*******************TT OBD ANX decode PROTOCOL*************************");
...
//             	System.out.println("**************************Comes under terminal*Register*************************");
         ...
         // Following part added on 12-02-2018 by Dev and Moin 
        	...
        	sendGeneralResponse(channel, remoteAddress, id, type, index);
               // PrintOut.PrintOutString("**************************end under terminal*heart BEAT*************************");
        }else if (type == MSG_TERMINAL_ATTRIBUTE)
        {
            ...
            StringBuilder terminalId = new StringBuilder();
                     terminalId.append(ByteBufUtil.hexDump(buf.readBytes(7)));
                     position.set("terminalId", terminalId.toString());
                        System.out.println("terminalId id------>"+terminalId.toString());
                 ...
                 int hardware_v_length =    buf.readUnsignedByte();
//                   System.out.println("hardware_v_length------>"+hardware_v_length);
                   
                   StringBuilder hardware_v_length_string = new StringBuilder();
                     hardware_v_length_string.append(ByteBufUtil.hexDump(buf.readBytes(4)));
                       position.set("hardware_version", hardware_v_length_string.toString());
//                   System.out.println("hardware_v_message------>"+hardware_v_length_string.toString());
                   
                      int firmware_Version_Num =    buf.readUnsignedByte();
//                   System.out.println("firmware_Version_Num------>"+firmware_Version_Num);
                   
                   StringBuilder firmware_Version_string = new StringBuilder();
                     firmware_Version_string.append(ByteBufUtil.hexDump(buf.readBytes(27)));
                        position.set("firmware_version", firmware_Version_string.toString());
//                   System.out.println("firmware_Version_string------>"+firmware_Version_string.toString());
                   
                  gnssModule(position, buf.readUnsignedByte());
                  
                    communicationModule(position, buf.readUnsignedByte());
...
//            buf.readerIndex(endIndex);
            ...
            System.out.println("getDeviceId()()"+getDeviceId());
                 ...
                 position.setBusiness_device_id(Context.getDataManager().getBusinessDeviceId((int)getDeviceId()));
                             System.out.println("setBusiness_device_id()()"+getDeviceId());
...
//    	 PrintOut.PrintOutString("**************************Comes under MSG LOCATION REPORT**************************");
    	        ...
    	        int flags = buf.readInt();
    	       boolean ignition = BitUtil.check(flags, 0);
    	        if(ignition){
    	        ...
    	        double lat = buf.readUnsignedInt() * 0.000001;
    	        double lon = buf.readUnsignedInt() * 0.000001;
              boolean latitude =  BitUtil.check(flags, 2);
    	        if (latitude) {
                     position.set("latitude_direction","S");
    	            position.setLatitude(-lat);
    	        } else {
                    position.set("latitude_direction","N");
    	            position.setLatitude(lat);
    	        }
                
                boolean longitude =  BitUtil.check(flags, 3);
    	        if (longitude) {
                      position.set("longitude_direction","W");
    	            position.setLongitude(-lon);
    	        } else {
                    position.set("longitude_direction","E");
    	            position.setLongitude(lon);
    	        }
    	        position.setAltitude(buf.readShort());
                ...
                case 0x31:
                 ...
                 case 0xEB:
                       ...
                       StringBuilder sb1 = new StringBuilder();
                                    sb1.append(ByteBufUtil.hexDump(buf.readBytes(10)));
                                    position.set("ICCID", sb1.toString());
                      System.out.println("ICCID------>"+sb1.toString());
                    break;
...
//    	        
//               buf.readShort();  
//                   buf.readInt();   // Extend status
//                buf.readShort();  // IO status
//                buf.readInt();    // Reserved
//                buf.readShort();  //Network signal
//                buf.readShort();  // GNSS satellite quantity
//                buf.readBytes(6);
