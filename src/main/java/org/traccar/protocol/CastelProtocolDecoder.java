...
public class CastelProtocolDecoder extends BaseProtocolDecoder {
    ...
    public static final short MSG_SC_Sensor = 0x4003;
    ...
    private Position readPosition(ByteBuf buf) throws Exception{
...
//        position.setSpeed(UnitsConverter.knotsFromCps(buf.readUnsignedShortLE()));
       position.setSpeed(buf.readUnsignedShortLE() * 0.036);
    ...
    public void DecodeStatus(long status, Position position) {
   //     System.out.println("DecodeStatus-------->" + status);
        if (BitUtil.check(status, 0)) {
            position.set(Position.KEY_STATUS, "Low_Voltage");
        }
        if (BitUtil.check(status, 1)) {
            position.set(Position.KEY_STATUS, "Towing");
        }
        if (BitUtil.check(status, 2)) {
            position.set(Position.KEY_STATUS, "Speeding");
        }
        if (BitUtil.check(status, 3)) {
            position.set(Position.KEY_STATUS, "Temperature");
        }
        if (BitUtil.check(status, 4)) {
            position.set(Position.KEY_STATUS, "Hard_Acceleration");
        }
        if (BitUtil.check(status, 5)) {
            position.set(Position.KEY_STATUS, "Hard_Deceleration");
        }
        if (BitUtil.check(status, 6)) {
            position.set(Position.KEY_STATUS, "Idle_Engine");
        }
        if (BitUtil.check(status, 7)) {
            position.set(Position.KEY_STATUS, "Exhaust_Emission");
        }
        if (BitUtil.check(status, 8)) {
            position.set(Position.KEY_STATUS, "High_RPM");
        }
        if (BitUtil.check(status, 9)) {
            position.set(Position.KEY_STATUS, "Power_ON");
        }
        if (BitUtil.check(status, 10)) {
            position.set(Position.KEY_STATUS, "Quick_Lane_Change");
        }
        if (BitUtil.check(status, 11)) {
            position.set(Position.KEY_STATUS, "Sharp_Turn");
        }
        if (BitUtil.check(status, 12)) {
            position.set(Position.KEY_STATUS, "Fatigue_Driving");
        }
        if (BitUtil.check(status, 13)) {
            position.set(Position.KEY_STATUS, "Emergency");
        }
        if (BitUtil.check(status, 14)) {
            position.set(Position.KEY_STATUS, "Crash");
        }
        if (BitUtil.check(status, 15)) {
            position.set(Position.KEY_STATUS, "Tamper");
        }
        if (BitUtil.check(status, 16)) {
            position.set(Position.KEY_STATUS, "Illegal_Enter");
        }
        if (BitUtil.check(status, 17)) {
            position.set(Position.KEY_STATUS, "Illegal_Ignition");
        }
        ...
        if (BitUtil.check(status, 19)) {
            position.set(Position.KEY_STATUS, "Privacy_Status");
        }
        if (BitUtil.check(status, 20)) {
            position.set(Position.KEY_STATUS, "No_GPS_Device");
        }
        if (BitUtil.check(status, 21)) {
            position.set(Position.KEY_STATUS, "Power_Off");
        }
        if (BitUtil.check(status, 22)) {
            position.set(Position.KEY_STATUS, "OBD_Communication_Error");
        }
        if (BitUtil.check(status, 23)) {
            position.set(Position.KEY_STATUS, "MIL");
        }
        if (BitUtil.check(status, 24)) {
            position.set(Position.KEY_STATUS, "Unlock");
        }
        if (BitUtil.check(status, 25)) {
            position.set(Position.KEY_STATUS, "No_Card_Presented");
        }
        if (BitUtil.check(status, 26)) {
            position.set(Position.KEY_STATUS, "Dangerous_Driving");
        }
        if (BitUtil.check(status, 27)) {
            position.set(Position.KEY_STATUS, "Vibration");
        }
        if (BitUtil.check(status, 28)) {
            position.set(Position.KEY_STATUS, "Reserved");
        }
        if (BitUtil.check(status, 29)) {
            position.set(Position.KEY_STATUS, "Reserved");
        }
        if (BitUtil.check(status, 30)) {
            position.set(Position.KEY_STATUS, "Reserved");
        }
    ...
    private Position createPosition() throws ClassNotFoundException, SQLException {
    ...
    private void decodeAlarm(Position position, int alarm) {
        switch (alarm) {
            ...
            case 0x17:
                ...
                break;
           case 0x1B:
                position.set(Position.KEY_ALARM, "dangerousDriving");
                break;
            case 0x11:
                position.set(Position.KEY_ALARM, "Crash");
                break;
            case 0x1C:
                position.set(Position.KEY_ALARM, "Vibration");
                break;
            default:
                break;
        }
...
//              position.set("total_mileage", total_mileage);
   ...
   private Object decodeSc(
            ...
            int version, ByteBuf id, short type) throws Exception{
        ...
        if (type == MSG_SC_HEARTBEAT) {
            ...
            long fuelConsumption = buf.readUnsignedIntLE();
              int fuel = buf.readUnsignedShort(); // current fuel consumption
...
//                position.setCurrent_mileage(tripOdometer);
//                   System.out.println("tripOdometer-------->"+tripOdometer);
//                Position last = getLastPosition(position.getDeviceId());
//                double c_m = tripOdometer - last.getCurrent_mileage();
//                double total_mileage = c_m + last.getTotal_mileage();
//                position.setTotal_mileage(total_mileage);
...
//                position.set(Position.KEY_STATUS, status);
                String hextype = Integer.toHexString(type);
//                  System.out.println("Package-------->"+hextype);
                   position.set("Package", hextype);
...
//              System.out.println("Package-------->"+hextype);
            ...
            String hextype = Integer.toHexString(type);
//              System.out.println("Package-------->"+hextype);
               position.set("Package",4008);
               
            decodeStat(position, buf);

            Network network = new Network(
                    CellTower.fromLacCid(buf.readUnsignedShortLE(), buf.readUnsignedShortLE()));
             position.set("carrier", network.getCarrier());
            position.set("radio_type", network.getRadioType());
            position.set("consider_id", network.getConsiderIp());
            position.set("home_mobile_country_code", network.getHomeMobileCountryCode());
            position.set("home_mobile_network_code", network.getHomeMobileNetworkCode());
...
//                 System.out.println("Package-------->"+hextype);
            ...
            for (int i = 0; i < successCount; i++) {
               int package_name =  buf.readUnsignedShortLE(); // tag
                String package_value =   buf.readSlice(buf.readUnsignedShortLE()).toString(StandardCharsets.US_ASCII);
                  int count = 0;
                if (package_name == 0x2001) {
                    count++;
                } else if (package_name == 0x1705) {
                    //   position.setVIN_no(package_value);
                    position.set(Position.KEY_RESULT, package_value);
                    return position;
                } else {
                    position.set(Position.KEY_RESULT, package_value);
                ...
                if (count > 0) {
                    position.setVin_number(package_value);
                    position.set("vin_number", package_value);
                    count = 0;
                } else {
                    position.setVin_number("00000");
                }
...
//                System.out.println("Package-------->"+hextype);
            ...
            buf.readUnsignedShort();
//                 System.out.println("....gSendor DAta sample_rate   ............" + buf.readUnsignedShort());
//                 System.out.println("....gSendor DAta group count   ............" + buf.readUnsignedByte());
//                 System.out.println("....gSendor DAta   x ............" + buf.readUnsignedShort());
            return position;
...
//         System.out.println("msg--------->"+buf);
        ...
        switch (version) {
            case -1:
                return decodeMpip(channel, remoteAddress, buf, version, id, type);
            case 3:
            case 4:
                return decodeSc(channel, remoteAddress, buf, version, id, type);
            default:
                return decodeCc(channel, remoteAddress, buf, version, id, type);
        }
    }
