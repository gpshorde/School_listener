...
public class TTo1ProtocolDecoder extends BaseProtocolDecoder {

    public TTo1ProtocolDecoder(Protocol protocol) {
        super(protocol);
    }
...
//         System.out.println("*******************HuaBao decode PROTOCOL*************************");
...
//                    System.out.println("spd location---------->"+spd);
            double speed = UnitsConverter.knotsFromKph(buf.readUnsignedShort() * 0.1) * 2;

                    position.setSpeed(speed);
                    
                    if(speed >= 5.0 && position.getIgnition() == 2)
                    {   
                        position.set(Position.KEY_ALARM, "towing");
                    }
