...
//    }
  ...
  private String concatenateStrings(String... strings) {
    ...
    protected DefaultFullHttpRequest request(String url) {
        return request(io.netty.handler.codec.http.HttpMethod.GET, url);
    }

    protected DefaultFullHttpRequest request(io.netty.handler.codec.http.HttpMethod method, String url) {
        return new DefaultFullHttpRequest(io.netty.handler.codec.http.HttpVersion.HTTP_1_1, method, url);
    }

    protected DefaultFullHttpRequest request(io.netty.handler.codec.http.HttpMethod method, String url, ByteBuf data) {
        return new DefaultFullHttpRequest(io.netty.handler.codec.http.HttpVersion.HTTP_1_1, method, url, data);
    }

    protected DefaultFullHttpRequest request(io.netty.handler.codec.http.HttpMethod method, String url, HttpHeaders headers) {
        return new DefaultFullHttpRequest(io.netty.handler.codec.http.HttpVersion.HTTP_1_1, method, url, Unpooled.buffer(), headers, new DefaultHttpHeaders());
    }
    ...
    protected void verifyFrame(ByteBuf expected, Object object) {
        ...
        Assert.assertNotNull("command is null", object);
        Assert.assertTrue("not a buffer", object instanceof ByteBuf);
        Assert.assertEquals(ByteBufUtil.hexDump(expected), ByteBufUtil.hexDump((ByteBuf) object));
